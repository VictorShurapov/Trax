...
class GPX: NSObject, XMLParserDelegate
{
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]) {
        ...
        case "rtept", "trkpt", "wpt":
            guard let latitude = attributeDict["lat"], let longitude = attributeDict["lon"] else { break }
            guard let latDouble = Double(latitude), let lonDouble = Double(longitude) else { break }
            waypoint = Waypoint(latitude: latDouble, longitude: lonDouble)
        case "link":
